...
@section('content')
    <section class="flex justify-between flex-wrap gap-2 mb-5 px-6 md:px-0">
        @if (session('error'))
            <p>{{ session('error') }}</p>
        @endif
        @if (session('success'))
        ...
        @endif
        <div>
            <h1 class="text-2xl font-bold">Properti Apartement</h1>
            <p class="text-lg text-slate-500">Manajemen dan kelola produk properti apartement Anda sekarang juga!</p>
        </div>
        <div>
            <a href="{{ route('pages.dashboard.properti.apartement.create') }}">
